...
embedding_matrix = pickle.load(open("../data/embedding_matrix.pkl", "r"))
word_index = pickle.load(open("../data/word_index.pkl", "r"))


max_features = len(word_index) + 1  # number of words we want to keep
...
#embeddings
embedding_layer = Embedding(max_features, embedding_dims, input_length=maxlen, embeddings_initializer=Constant(embedding_matrix), trainable=True)
...
#cd kaggleN	trained_model = malstm.fit([np.concatenate((x_train['left'], x_test['left'])), np.concatenate((x_train['right'], x_test['right']))], np.concatenate((y_train,y_test)), batch_size=batch_size, epochs=n_epoch, validation_split=0.1)
#malstm.save('malstm-pretrained.h5') 
# malstm.save_weights('malstm-pretrained_w.h5')
malstm.load_weights('malstm-pretrained_w.h5')
# ## testing
...
#print submission
submit_file_path = "/home/palash/kaggle/NLP/doc-similarity/data/submission-2.csv"
